...
def to_result(key, args, parent_key=None):
    """
    Examination is unusual. argsConvert to Unified Data
    :param key:       Examinationkey
    :param args:      Unusual exam parameters.
    :param parent_key Fatherkey
    ...
    error_detail = list(filter(
        lambda d: True if isinstance(d, ErrorDetail) else True if isinstance(d, dict) and len(
            d.keys()) > 0 else False,
        (args[0] if len(args) > 0 else {key: [ErrorDetail('Unknown Unusual', code='unknown')]}).get(key)))[0]
    ...
    return result.Result(500 if isinstance(error_detail.code, str) else error_detail.code,
                         message=f"【{key if parent_key is None else parent_key + '.' + key}】To fill the parameters." if str(
                             error_detail) == "This field is required." else error_detail)


def validation_error_to_result(exc: ValidationError):
    """
    Unusual reacting objects.
    :param exc: Examples are unusual.
    :return: Interface Response Objects
    """
    try:
...
def handle_exception(exc, context):
    ...
    # Calling first.REST frameworkdefault abnormal treatment methods get standard error response objects
    response = exception_handler(exc, context)
    # Additionally customized unusual treatment here.
