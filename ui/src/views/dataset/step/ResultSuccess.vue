<template>
  <el-scrollbar>
    <el-result icon="success" title="🎉 Knowledge Base Success 🎉">
      <template #sub-title>
        <div class="mt-8">
          <span class="bold">{{ data?.document_list.length || 0 }}</span>
          <el-text type="info" class="ml-4">Documents</el-text>
          <el-divider direction="vertical" />
          <span class="bold">{{ paragraph_count || 0 }}</span>
          <el-text type="info" class="ml-4">Parts</el-text>
          <el-divider direction="vertical" />
          <span class="bold">{{ numberFormat(char_length) || 0 }}</span>
          <el-text type="info" class="ml-4">The characters</el-text>
        </div>
      </template>
      <template #extra>
        <el-button @click="router.push({ path: `/dataset` })">Return to Knowledge Base List</el-button>
        <el-button type="primary" @click="router.push({ path: `/dataset/${data?.id}/document` })"
          >Go to documentation.</el-button
        >
      </template>
    </el-result>
    <div class="result-success">
      <p class="bolder">List of documents</p>
      <el-card
        shadow="never"
      ...
      >
        <div class="flex-between">
          ...
          <div>
            <el-text type="info" class="mr-16">{{ item && item?.paragraph_count }} one part.</el-text>
